...
using static IoTClient.Clients.CAN.zlgcan;

namespace IoTClient.Clients.CAN
{
    class recvdatathread
    {
        ...
        {
            ZCAN_Receive_Data[] can_data = new ZCAN_Receive_Data[1000];
            ZCAN_ReceiveFD_Data[] canfd_data = new ZCAN_ReceiveFD_Data[1000];
            uint len;
            while (m_bStart)
            {
                lock (locker)
